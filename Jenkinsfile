pipeline {
  ...
  stages {
    ...
    stage ('Trivy') {
      steps {
        // Install trivy
        sh 'sudo -s curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sh -s -- -b /usr/local/bin v0.18.3'
        sh 'sudo -s curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/html.tpl > html.tpl'
      }
    }
    stage ('CommitSemgrep') {
      steps {
